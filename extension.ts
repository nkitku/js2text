...
(<any>global)._ = _;
(<any>global).math = math;
(<any>global).n2w = (function() {
  const n = (function() {
    ...
    let n = ["Twenty", "Thirty", "Forty", "Fifty", "Sixty", "Seventy", "Eighty", "Ninety"].map(v =>
      o.map(x => v + (x && " " + x))
    );
    return _([o, t, ...n])
      .flatten()
  ...
  const d3 = (x, y = ("000" + x).slice(-3)) =>
    `${+y[0] ? n[y[0]] + " Hundred" : ""} ${+y[1] + +y[2] ? n[+(y[1] + y[2])] : ""}`.trim();
  return (x, y = ((x += ""), "0".repeat(3 - (x.length % 3)) + x)) =>
    y
      .match(/.{3}/g)
      .map((v, i, k) => (d3(v) && d3(v) + " " + fg[k.length - i - 1]).trim())
      .join(" ")
      .trim();
})();
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const mathJs2text = vscode.commands.registerTextEditorCommand("extension.mathJs2text", (textEditor, edit) => {
    const selectionsTransformed = textEditor.selections.map(selection => {
      const text = textEditor.document.getText(selection);
